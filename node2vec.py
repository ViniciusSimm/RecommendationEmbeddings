...
def objective(trial):
    ...
    p = trial.suggest_float('p', 0.25, 4.0, log=True)  # Controla a propensão de voltar ao nó anterior
    q = trial.suggest_float('q', 0.25, 4.0, log=True)  # Controla a propensão de explorar novos nós

    print(f'This version: vector size: {vector_size}, walk_length: {walk_length}, context_size: {context_size}, walks_per_node: {walks_per_node}, num_negative_samples: {num_negative_samples}, q: {q}, p:{p}')
    ...
    with open('node2vec.txt', 'a') as f:
        row = f'vector_size={vector_size}, walk_length={walk_length}, context_size={context_size}, walks_per_node={walks_per_node}, num_negative_samples={num_negative_samples}, p={p}, q={q}, score={score}'
        f.write(row + '\n')

    return score


storage_name = "sqlite:///node2vec.db"

study = optuna.create_study(direction='maximize', storage=storage_name, study_name="node2vec", load_if_exists=True)
